...
export const labels: Partial<Record<TriggerId, { name: string; icon: string; hint: string }>> = {
  ...
  "alarm-siren": { name: "Alarms & high tones", icon: "◌", hint: "Persistent multi-tone tracking for house alarms, sirens, and piercing beeps" },
  "glass-shatter": { name: "Glass & brittle crashes", icon: "◇", hint: "High-flux transient routing for glass breaks and sharp brittle impacts" },
...
const initialRules: TriggerRule[] = [
  ...
  { id: "glass-shatter", enabled: true, strength: 74 },
...
function engineName(status: Extract<EngineStatus, { state: "protecting" }>): string {
  if (status.engine === "gtcrn") return "GTCRN neural + competitive event routing";
  if (status.engine === "rnnoise") return "RNNoise neural + competitive event routing";
  return "Competitive sensory event routing";
}
...
export function App({ runtime = chromeRuntime }: { runtime?: UiRuntime }) {
  ...
  return <main className="shell">
    ...
    <section className="hero" aria-labelledby="page-title">
      <div><p className="eyebrow">SENSORY PROTECTION</p><h1 id="page-title">Make sound<br /><em>work for you.</em></h1><p className="intro">Route background noise, alarms, glass breaks, clatter, harsh highs, and sudden loudness into different local protection paths instead of flattening the whole tab.</p></div>
